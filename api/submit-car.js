...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Basic validation
    if (!email || !carName || !teamName) {
      return res.status(400).json({ message: 'Missing required fields' });
    }

    // JSON attachment with car properties
    ...
    await transporter.sendMail({
      ...
      attachments: [
        ...
        { filename: 'car.png', content: carImage.split('base64,')[1], encoding: 'base64' },
        { filename: 'wheel.png', content: wheelImage.split('base64,')[1], encoding: 'base64' }
      ]
    });
